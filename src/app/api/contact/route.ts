...
export async function POST(req: Request) {
  try {
    ...
    const contactValidation = validatePreferredContact({
    ...
    if (!contactValidation.ok) {
      return NextResponse.json(
        { ok: false, error: contactValidation.error },
        { status: 400 }
      );
    }

    const hasEmail = contactValidation.hasEmail;
    const shouldSendUserConfirm = hasEmail;
